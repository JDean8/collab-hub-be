...
exports.postSingleChatMessage = (chat_id: string, message: string, user_id: string, avatar_url: string) => {
    ...
    if(!chat_id || !message || !user_id || !avatar_url) return Promise.reject({status: 400, msg: "Bad request"})
...
exports.postMember = (chat_id: string, user_id: string) => {
    if(!chat_id || !user_id) return Promise.reject({status: 400, msg: "Bad request"})
    return db.query("INSERT INTO chat_members (chat_id, user_id) VALUES ($1, $2) RETURNING *;", [chat_id, user_id])
